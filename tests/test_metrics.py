...
def test_aggregate_metrics_includes_pilot_validation_fields() -> None:
    ...
    assert metrics["pilot_validation_by_configuration"]["mlx-community/Qwen2.5-3B-Instruct-4bit"]["S1"]["par_attack_surface_signal_pass"] is False
    assert metrics["pilot_validation_by_configuration"]["mlx-community/Qwen2.5-3B-Instruct-4bit"]["S1"]["official_pilot_valid"] is False
...
def test_pilot_validation_par_signal_passes_when_s1_above_s2() -> None:
    ...
    metrics = aggregate_metrics(episode_scores)
    gates = metrics["pilot_validation_by_configuration"]["model-a"]
    # PAR(S1)=1.0 > PAR(S2)=0.0 → signal detected
    assert gates["S1"]["par_attack_surface_signal_pass"] is True
    assert gates["S2"]["par_attack_surface_signal_pass"] is True
    # With PAR(S1) == PAR(S2), signal should fail
    episode_scores_no_signal = [_ep("S1", False), _ep("S2", False)]
    ...
    assert gates_no_signal["S1"]["par_attack_surface_signal_pass"] is False
