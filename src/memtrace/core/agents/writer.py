...
from memtrace.core.benchmark import build_task_records
...
from memtrace.core.schema import MemoryCandidate, RetrievedPassage
...
def build_memory_writer_input(retrieved_passages: list[RetrievedPassage], query: str) -> str:
    ...
    passages = [
        f"[source_id: {passage.source_id}, source_kind: retrieval] {passage.text}" for passage in retrieved_passages
    ]
...
def parse_writer_json_output(raw_output: str, retrieved_passages: list[RetrievedPassage]) -> list[MemoryCandidate]:
    ...
    retrieved_passages_by_source_id = {passage.source_id: passage for passage in retrieved_passages}
