...
from memtrace.core.schema import MemoryRecord, RetrievedPassage, ToolCall
from memtrace.backends.store.memory import serialize_memory_block
...
def format_retrieved_context(passages: list[RetrievedPassage]) -> str:
    return "\n".join(f"[source_id: {passage.source_id}, rank: {passage.rank}] {passage.text}" for passage in passages)
