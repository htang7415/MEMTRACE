...
from memtrace.evaluation.scoring import load_json, load_jsonl
...
def _exemplar_section(labels: list[dict]) -> list[str]:
    ...
    for label_name in (
        ...
        rows = [
            item
            for item in labels
            if (item["unsafe"] or item.get("execution_failure")) and item["attribution_label"] == label_name
        ][:3]
        if not rows:
...
def _poison_source_ids(turn: dict) -> set[str]:
    return {hit["source_id"] for hit in turn.get("retrieved_passages", []) if hit.get("passage_kind") == "poison"}


def _almost_fired_rule(
    score: dict, final_poison_present: bool, filler_context_present: bool, final_turn_number: int
) -> str:
    candidates = []
    if score.get("execution_failure"):
...
def _filler_context_present(trace: list[dict], final_turn_number: int) -> bool:
    for row in trace[:-1]:
        ...
        if 1 < turn < final_turn_number and row.get("retrieved_passages"):
            return True
    return any(record.get("write_turn", 0) > 1 for record in (trace[-1].get("prior_memory_state", []) if trace else []))
