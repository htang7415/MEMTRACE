...
def main() -> None:
    ...
    outputs = {
        ...
        "figure5": _save_figure(
            _draw_violation_by_horizon(episode_scores), figures_dir / "figure5_violation_by_horizon"
        ),
    }
...
def _draw_ovr_vs_svr(metrics: dict, calibration_metrics: dict | None = None):
    ...
    for index, (label, value, color) in enumerate(path_rows):
        ...
        _label(
            ax, 410, y_pos + 11.5, label, size=9.5, weight="bold" if index in {1, 2, 3, 4} else "normal", va="center"
        )
    ...
    if calibration:
        ...
        _compact_bar(
            ...
            354,
            "Cal.",
            [calibration["unsafe_executed"], calibration["episodes"]],
            PALETTE["green"],
            y_max=0.12,
            track_w=90,
        )
...
def _draw_causal_chain_by_system(metrics: dict):
    ...
    _label(
        ...
        46,
        330,
        f"Stateful execution-format failures: {failures} per system, tracked separately.",
        size=10,
        color=PALETTE["muted"],
    )
    return fig
...
def _draw_task_localization(episode_scores: list[dict]):
    ...
    for task_id in sorted({item["task_id"] for item in episode_scores if item["episode_kind"] == "stateful_attack"}):
        items = [
            ...
            if item["system"] == "S1" and item["episode_kind"] == "stateful_attack" and item["task_id"] == task_id
        ]
        rows.append(
            (
                task_id,
    ...
    _label(
        ax, 852, 523, "Unsafe proposal/execution is 0/6 for every task.", size=8.5, color=PALETTE["muted"], ha="right"
    )
    return fig
    ...
    for row_index, (label, key, color) in enumerate(stages):
        ...
        for col, horizon in enumerate(horizons):
            ...
            _shadowed_rounded(
                ...
                cell_h,
                face=color,
                edge=color if count else PALETTE["grid"],
                alpha=alpha,
                radius=8,
                shadow_alpha=0.10,
            )
...
def _s1_horizon_diagnostics(episode_scores: list[dict]) -> dict[int, dict[str, int]]:
    ...
    for horizon in (1, 3, 7):
        items = [
            item
            for item in episode_scores
            if item["system"] == "S1" and item["episode_kind"] == "stateful_attack" and item["horizon"] == horizon
        ]
        rows[horizon] = {
            "denominator": len(items),
...
def _summary_panel(ax, x: float, y: float, width: float, height: float, title: str) -> None:
    ...
    _shadowed_rounded(
        ax, x, y, width, height, face=PALETTE["white"], edge=PALETTE["grid"], radius=12, shadow_alpha=0.12
    )
