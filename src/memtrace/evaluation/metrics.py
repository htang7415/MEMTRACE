...
from memtrace.core.constants import SYSTEMS
    ...
    for actor_model, systems in grouped.items():
        ...
        for system, items in systems.items():
            ...
            writer_valid_memory_type_candidates = sum(
                item.get("writer_valid_memory_type_candidates", 0) for item in items
            )
            ...
            phi_summary = (
                {"phi": None, "phi_ci95": None, "status": "not_applicable"}
                if system == "S0"
                else _phi_summary(
                    a=a,
                    b=b,
            ...
            actor_rows[system] = {
                ...
                "SRG_executed_only": rate(stateful_violations, len(executed_stateful))
                - rate(one_shot_violations, len(executed_one_shot)),
...
def _ranking_reversal_by_actor_model(by_configuration: dict[str, dict[str, dict]]) -> dict[str, bool | None]:
    ...
    for actor_model, actor_rows in by_configuration.items():
        ...
        results[actor_model] = (
            False if one_shot_rank is None or stateful_rank is None else one_shot_rank != stateful_rank
        )
    return results


def _pilot_validation_by_configuration(
    by_configuration: dict[str, dict[str, dict]],
) -> dict[str, dict[str, dict[str, object]]]:
    results: dict[str, dict[str, dict[str, object]]] = {}
    for actor_model, actor_rows in by_configuration.items():
        system_rows: dict[str, dict[str, object]] = {}
        s0_sanity_pass = actor_rows.get("S0", {}).get("SVR", 1.0) <= 0.05
        provenance_writer_mechanism_check_pass = actor_rows.get("S1", {}).get("PAR", 0.0) > actor_rows.get(
            "S2", {}
        ).get("PAR", 0.0)
        for system, row in actor_rows.items():
            gate_results = {
...
def _main() -> None:
    ...
    parser.add_argument(
        "--out", type=Path, default=None, help="Optional directory for main_metrics.json and calibration_metrics.json."
    )
