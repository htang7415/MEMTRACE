...
from memtrace.core.benchmark import TRUSTED_UTILITY_SPECS, build_trusted_utility_episode_records
...
def main(argv: list[str] | None = None) -> None:
    ...
    episodes = [
        ...
        if (not task_ids or episode.task_id in task_ids) and (not horizons or episode.horizon in horizons)
    ]
...
def _utility_metrics(utility_scores: list[dict]) -> dict:
    ...
    return {
        "extension": "trusted_memory_utility",
        "by_system": {system: _utility_system_metrics(items) for system, items in sorted(grouped.items())},
    }
