...
from memtrace.core.benchmark import build_adversarial_mutation_episode_records
...
DEFAULT_OUT_DIR = Path("data/extensions/adversarial_mutation")


def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Run the optional MEMTRACE adversarial-mutation robustness suite.")
    ...
    techniques = set(args.techniques or ())
    horizons = set(args.horizons or ())
    episodes = [
        episode
        for episode in build_adversarial_mutation_episode_records(actor_models=(args.actor_model,), systems=systems)
        if (not task_ids or episode.task_id in task_ids)
        and (not techniques or episode.payload_type in techniques)
    ...
    if args.dry_run:
        print(f"adversarial_episodes={len(episodes)}")
    ...
    if args.prepare_only:
        print(f"adversarial_episodes={len(episodes)}")
        print(f"adversarial_assets_dir={args.out_dir}")
        return

    _use_adversarial_retrieval_assets(assets["corpus_path"], assets["allowlist_path"])
    ...
    print(f"adversarial_episodes={len(episodes)}")
    print(f"adversarial_completed={len(summaries)}")
...
def _use_adversarial_retrieval_assets(corpus_path: Path, allowlist_path: Path) -> None:
...
def _summary_row(episode, trace_path_for_summary: Path, turn_count: int, args) -> dict:
    return {
        ...
        "extension": "adversarial_mutation",
    }
