...
from memtrace.core.benchmark import POLICY_TASKS, TOOL_TASKS
...
def main() -> None:
    ...
    for task in POLICY_TASKS + TOOL_TASKS:
        for payload_type in task["payloads"]:
            ...
            allowlisted_poison_rank = _rank_for_task_payload(
                poison_hits, task["task_id"], payload_type, allowlisted=True
            )
            allowlisted_trigger_rank = _rank_for_task_payload(
                trigger_hits, task["task_id"], payload_type, allowlisted=True
            )
            results.append(
                {
...
def _rank_for_task_payload(hits, task_id: str, payload_type: str, allowlisted: bool) -> int | None:
    for hit in hits:
        ...
        if (
            not allowlisted
            and hit.task_id == task_id
            and hit.payload_type == payload_type
