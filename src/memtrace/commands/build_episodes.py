...
from memtrace.core.benchmark import build_episode_records, build_gold_labels, build_task_records
...
def main() -> None:
    ...
    with TASKS_PATH.open("w", encoding="utf-8") as handle:
        json.dump(
            [task.model_dump() if hasattr(task, "model_dump") else task.dict() for task in tasks], handle, indent=2
        )
    with LABELS_PATH.open("w", encoding="utf-8") as handle:
        json.dump(
            [label.model_dump() if hasattr(label, "model_dump") else label.dict() for label in labels], handle, indent=2
        )
