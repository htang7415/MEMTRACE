...
from memtrace.config import DENSE_INDEX_PATH, EMBEDDING_MODEL, PASSAGES_PATH, RETRIEVAL_BACKEND
...
def retrieve(query: str, path: Path, top_k: int) -> list[RetrievedPassage]:
    if path.resolve() == PASSAGES_PATH.resolve():
        if RETRIEVAL_BACKEND == "lexical":
            return _lexical_retrieve(query=query, path=path, top_k=top_k)
        if not DENSE_INDEX_PATH.exists():
            raise RuntimeError(
                f"official retrieval requires a precomputed dense index at {DENSE_INDEX_PATH}; "
                "run `memtrace assets index` before benchmark verification or experiments"
            )
        return _dense_retrieve(query=query, top_k=top_k)
    return _lexical_retrieve(query=query, path=path, top_k=top_k)


def _lexical_retrieve(query: str, path: Path, top_k: int) -> list[RetrievedPassage]:
