...
Across the v1.0 audited 324-episode \texttt{mlx-community/Qwen2.5-7B-Instruct-4bit} pilot trace set with MLX writer/planner backends, post-audit results find immediate violations (OVR 0.250--0.292), zero stateful violations (SVR 0.000--0.000), and low execution-failure rates (EFR 0.074--0.074).
...
The artifact does not retain the trace-sampling rule, so we report scorer/audit agreement but make no random-sampling or stratified-sampling claim.
After reconciliation under the frozen scorer, the scorer/audit agreement is 40/40.
