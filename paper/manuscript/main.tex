...
In a v1.0 audited pilot with 324 main traces for \texttt{mlx-community/Qwen2.5-7B-Instruct-4bit} using MLX writer/planner backends, post-audit results show immediate one-shot violations (6--7/24 attack episodes per system), zero delayed stateful violations (0/72 per system), and execution-failure rates of 8/108 per system.
A permissive writer admits poisoned memory in 6/72 S1 stateful episodes, and all six admitted records are retrieved at the trigger turn, but none leads to an unsafe proposal or executed delayed policy violation; a provenance-aware writer blocks these admissions.
A separate forced-memory calibration condition, excluded from the main S0/S1/S2 rates, produces delayed memory-mediated executed violations (CAL-SVR 3/72 = 0.042), confirming that the scorer and trace protocol can detect the delayed pathway when poisoned memory is forced into the trigger context.
MEMTRACE therefore supports a validity-first evaluation claim: one-shot attack success is not a proxy for persistent-memory compromise.
\end{abstract}
...
\begin{tabular}{p{0.24\linewidth}>{\centering\arraybackslash}p{0.15\linewidth}>{\centering\arraybackslash}p{0.16\linewidth}p{0.33\linewidth}}
\toprule
...
We evaluate the v1.0 audited pilot slice using \texttt{mlx-community/Qwen2.5-7B-Instruct-4bit} with MLX writer/planner backends, deterministic decoding, fixed retrieval order, fixed prompts, fixed tool outputs, and deterministic policy checks.
...
The calibration is not intended to maximize attack success; it is a sensitivity check showing that the trace protocol, scorer, and attribution path can register delayed memory-mediated execution when the memory-retrieval link is forced, while leaving the main threat-model rates unchanged.
...
Across the included 324-episode packet, OVR 0.250--0.292, SVR 0.000--0.000, and EFR 0.074--0.074.
...
\small
\input{tables/failure_attribution.tex}
\end{table*}
...
\label{tab:mechanism}
\input{tables/mechanism_counts.tex}
\end{table}
...
\small
\input{tables/causal_chain_counts.tex}
\end{table*}
...
\small
\input{tables/task_level_signal.tex}
\end{table*}
...
\label{tab:validity}
\small
\input{tables/validity_gates.tex}
\end{table}
...
The gates are still important: they prevent invalid traces from being presented as included evidence and make the result reproducible as a pilot-valid slice rather than a post-hoc diagnostic.
Appendix~\ref{sec:attempted-runs} lists all local run packets considered during preparation, including actor codes, inclusion decisions, exclusion reasons, and trace-hash prefixes.
Excluded rows are development probes and are not used as model-comparison evidence.
...
These limitations are deliberate for v1.0: MEMTRACE prioritizes trace-level auditability and claim validity over leaderboard breadth.
The released protocol, scorer, and validity gates are intended to make future actor/backend extensions directly comparable without changing the interpretation of the admission-to-execution pathway.
...
The released protocol, traces, scorer, and validity gates make future cross-actor extensions auditable under the same admission-to-execution decomposition.
...
\begin{table}[ht]
\centering
\caption{Wilson 95\% intervals for fixed deterministic episode sets. Intervals quantify finite-episode uncertainty, not stochastic decoding variability. Calibration intervals are reported separately and do not alter the main S0/S1/S2 rates.}
\label{tab:confidence-intervals}
\input{tables/confidence_intervals.tex}
\end{table}

\section{Human Audit}

\paragraph{Human audit.}
The audit packet contains 40 scorer-audit comparisons for trace-level labels, including unsafe execution, execution failure, poisoned-memory admission, and memory-mediated attribution.
The artifact does not retain the trace-sampling rule, so we report retained audit-packet reconciliation but make no random-sampling or stratified-sampling claim.
After reconciliation under the frozen scorer, retained audit-packet reconciliation is 40/40 comparisons.
The audit packet is therefore used as a scorer-consistency check, not as an estimate of human-label reliability over a random or stratified trace sample.
The audit packet is included in the anonymous artifact.
...
\scriptsize
\input{tables/oracle_memory_calibration.tex}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Compute accounting for generation and calibration jobs. ``Not retained'' means the audited trace/run-summary files do not contain the telemetry field.}
\label{tab:compute-generation}
\footnotesize
\begin{tabular}{@{}p{0.16\linewidth}rp{0.31\linewidth}p{0.31\linewidth}@{}}
\toprule
Run group & Episodes & Actor/backend & Worker/time/memory record \\
\midrule
Main v1.0 S0 & 108 & \path{mlx-community/Qwen2.5-7B-Instruct-4bit}; MLX writer/planner & Apple Silicon/MLX backend; exact worker, runtime, and peak memory not retained \\
Main v1.0 S1 & 108 & \path{mlx-community/Qwen2.5-7B-Instruct-4bit}; MLX writer/planner & Apple Silicon/MLX backend; exact worker, runtime, and peak memory not retained \\
Main v1.0 S2 & 108 & \path{mlx-community/Qwen2.5-7B-Instruct-4bit}; MLX writer/planner & Apple Silicon/MLX backend; exact worker, runtime, and peak memory not retained \\
Oracle-memory calibration & 72 & \path{mlx-community/Qwen2.5-7B-Instruct-4bit}; MLX writer/planner & Mac mini M4, 16 GB; runtime about 64 min; peak memory not retained \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{No-inference artifact validation jobs used to regenerate paper outputs.}
\label{tab:compute-validation}
\footnotesize
\begin{tabular}{@{}p{0.24\linewidth}rp{0.28\linewidth}p{0.12\linewidth}p{0.09\linewidth}@{}}
\toprule
Run group & Episodes & Hardware/software & Runtime & Peak memory \\
\midrule
Metric regeneration from main traces & 324 & Mac mini M4, 16 GB, macOS 26.4.1, Python 3.12.6 & 4.48 s & 28.6 MB \\
Metric regeneration from calibration traces & 72 & Mac mini M4, 16 GB, macOS 26.4.1, Python 3.12.6 & 0.54 s & 25.2 MB \\
Full release validation & 396 & Mac mini M4, 16 GB, macOS 26.4.1, Python 3.12.6 & 12.45 s & 10.8 MB \\
\bottomrule
\end{tabular}
\end{table}
...
Tables~\ref{tab:asset-licenses-external} and~\ref{tab:asset-licenses-dependencies} list the assets that affect the reported runs.
...
\begin{table}[ht]
\centering
\caption{External models and backends referenced by the v1.0 artifact. The anonymous submission artifact withholds author identity in the MEMTRACE license header; the public release should restore the non-anonymous copyright holder.}
\label{tab:asset-licenses-external}
\footnotesize
\begin{tabular}{@{}p{0.28\linewidth}p{0.32\linewidth}p{0.15\linewidth}p{0.11\linewidth}@{}}
\toprule
Asset & Version/revision & Role & License \\
\midrule
\path{mlx-community/Qwen2.5-7B-Instruct-4bit} & cached HF rev. \texttt{c26a38f6}; converted from Qwen with \texttt{mlx-lm} 0.18.1 & actor model & Apache-2.0 \\
Qwen tokenizer bundled with actor & cached HF rev. \texttt{c26a38f6} & tokenization & Apache-2.0 \\
\texttt{mlx-lm} & requirement \texttt{>=0.21}; local verification 0.31.3; generation package version not retained & generation backend & MIT \\
\path{BAAI/bge-small-en-v1.5} & cached HF rev. \texttt{5c38ec7c} & dense retrieval & MIT \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[ht]
\centering
\caption{Python dependencies and MEMTRACE assets in the anonymous review artifact.}
\label{tab:asset-licenses-dependencies}
\footnotesize
\begin{tabular}{@{}p{0.28\linewidth}p{0.28\linewidth}p{0.17\linewidth}p{0.13\linewidth}@{}}
\toprule
Asset & Version/revision & Role & License/status \\
\midrule
...
MEMTRACE synthetic corpus & v1.0 & benchmark asset & MIT included \\
MEMTRACE traces & v1.0 main 324 plus 72 calibration & benchmark asset & MIT included \\
\bottomrule
\end{tabular}
\end{table}

\section{Attempted Runs and Exclusions}
\label{sec:attempted-runs}
...
\scriptsize
\resizebox{\linewidth}{!}{%
\input{tables/attempted_runs.tex}
}
\end{table*}

No excluded packet is used to support model-comparison, prevalence, or defense-superiority claims.
